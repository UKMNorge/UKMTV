...
if ( !$cache_id ) {
	$sql = new SQLins('ukmtv_caches_caches');
	$sql->add('ip', $cache_ip);
	$sql->add('status', 'OK');
	$results = $sql->run();
	$cache_id = $sql->insid();
} else {
	$select_sql = new SQL("SELECT id FROM `ukmtv_caches_caches` WHERE `id`='#id'", array('id' => $cache_id));
	$res = $select_sql->run( $select_sql );
	$res = mysql_fetch_assoc( $res );
	if ( !$res ) {
		error_log("Got heartbeat from unknown id. ID=$cache_id, ip=$cache_ip.");
		$insert_new = new SQLins('ukmtv_caches_caches');
	...
	$sql = new SQLins('ukmtv_caches_caches', array('id' => $cache_id));
	$sql->add('ip', $cache_ip);
	$sql->run();
}
